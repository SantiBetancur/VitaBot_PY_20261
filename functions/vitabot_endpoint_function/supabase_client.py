...
def rpc(schema, function_name, params):
    """
    RPC: Llama a una función PostgreSQL expuesta por Supabase.

    Parámetros:
        schema: nombre del schema donde vive la función (ej: "rag")
        function_name: nombre de la función (ej: "match_documents")
        params: dict con los argumentos de la función

    Retorna:
        Lista con los registros devueltos por la función
    """
    ...
    try:
        ...
        logger.error(f"Error en RPC {function_name}: {str(e)}")
